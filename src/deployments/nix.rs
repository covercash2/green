...
use crate::{io::IoError, text::Span};
...
pub enum NixError {
    ...
    #[allow(dead_code)] // not yet constructed; kept for parity with RevNotFound
    RevExtractFailed,
...
#[derive(Debug, Clone)]
pub struct SystemFlake {
    path: PathBuf,
    contents: String,
}

impl SystemFlake {
    pub async fn load(path: impl AsRef<Path>) -> Result<Self, NixError> {
        let path_buf = path.as_ref().to_path_buf();
        let contents = crate::io::read_file(&path_buf).await?;
    ...
    pub async fn test() -> Self {
        let path: PathBuf = "./fixtures/good_ultron.flake.nix"
            .parse()
            .expect("failed to parse path");

        Self::load(&path).await.expect("failed to load test flake")
    }
}

/// find the `ultron.rev` and update it to `new_rev`
pub fn update_ultron_rev(input: &str, new_rev: &str) -> Result<String, NixError> {
