...
/// Deserialize a YAML value that is either a sequence of strings or a bare
/// string (optionally comma-delimited) into `Vec<String>`.
///
/// Obsidian frontmatter allows all of:
/// - `tags: [NPC, world]`  — YAML sequence
    ...
    D: serde::Deserializer<'de>,
{
    ...
    impl<'de> Visitor<'de> for TagsVisitor {
        ...
        // `aliases:` or `tags:` with no value → YAML null → unit
        fn visit_unit<E: serde::de::Error>(self) -> Result<Vec<String>, E> {
            Ok(Vec::new())
        }

        fn visit_none<E: serde::de::Error>(self) -> Result<Vec<String>, E> {
            Ok(Vec::new())
        }
