...
impl Index {
    pub async fn new(routes: Routes, has_notes: bool, has_mqtt: bool, has_mqtt_devices: bool, has_logs: bool, has_services: bool) -> Result<Self, Error> {
        let static_entries = [
            IndexEntry {
        ...
        let services_entry = has_services.then_some(IndexEntry {
            name: "services".into(),
            href: "/services".into(),
            description: "Systemd service status".into(),
        });
...
mod tests {
    ...
    async fn index_with_notes_has_notes_entry() {
        let index = Index::new(Routes::default(), true, false, false, false, false).await.unwrap();
        assert!(
            index.routes.iter().any(|r| r.href == "/notes"),
    ...
    async fn index_without_mqtt_devices_has_no_devices_entry() {
        let index = Index::new(Routes::default(), false, true, false, false, false).await.unwrap();
        assert!(
    ...
    async fn index_with_mqtt_devices_has_devices_entry() {
        let index = Index::new(Routes::default(), false, true, true, false, false).await.unwrap();
        assert!(
            index.routes.iter().any(|r| r.href == "/mqtt/devices"),
